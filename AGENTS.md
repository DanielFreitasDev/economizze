...
This repository is a Spring Boot application (Java 21) built with Maven Wrapper.

- Application code: `src/main/java/io/nexus/economizze`
- Configuration: `src/main/resources/application.properties`
- DB migration location (Flyway): `src/main/resources/db/migration`
- Web assets and templates: `src/main/resources/static`, `src/main/resources/templates`
- Tests: `src/test/java/io/nexus/economizze`

As features are added, prefer organizing packages by feature/domain (for example `transaction`, `category`, `budget`) instead of a single flat package.

## Build, Test, and Development Commands
Use the Maven Wrapper so all contributors run the same Maven version:

- `./mvnw spring-boot:run`: run the app locally
- `./mvnw test`: run automated tests
- `./mvnw clean verify`: full validation lifecycle (compile, test, package checks)
- `./mvnw clean package`: produce the JAR in `target/`

Default datasource points to local PostgreSQL (`economizze` on `localhost:5432`), configured in `application.properties`.

## Coding Style & Naming Conventions
Follow standard Java + Spring conventions and keep formatting consistent with existing files.

- Classes/interfaces: `PascalCase`
- Methods/fields: `camelCase`
- Constants: `UPPER_SNAKE_CASE`
- Test classes: suffix with `Test` or `Tests`

Prefer constructor injection for Spring beans. Keep controllers thin, business rules in services, and persistence concerns in repositories/entities.

## Testing Guidelines
Testing uses JUnit 5 via `spring-boot-starter-test`.

- Put tests under `src/test/java` matching production package structure.
- Use `@SpringBootTest` for context/integration tests.
- Use focused unit tests for business logic when full context is not required.
- Add tests for new behavior and bug fixes before opening a PR.

No coverage gate is currently enforced; contributors should still add meaningful tests for changed code paths.

## Commit & Pull Request Guidelines
Current history is minimal (`Commit inicial`), with short subject-line commits. Keep that style, but make messages specific and imperative.

- Commit subject: one concise line (prefer <= 72 chars)
- Suggested pattern: `feat: add transaction validation`
...
- Migration/config notes when touching database or properties

## Security & Configuration Tips
Do not commit real credentials. Keep local defaults in `application.properties` and use environment variables or profile-specific files for non-local environments.
